<#
.SYNOPSIS
    Per-user AD provisioning: stage, assign ID, and sort one hire at a time.

.DESCRIPTION
    Each pending row from the HR spreadsheet is processed individually through
    these jobs in order:

        1. Grab user      - pick the next unprocessed spreadsheet row
        2. Stage user     - create (or locate) the AD account in 1NewUserStaging
        3. Assign ID      - assign the next sequential EmployeeID (EMP1001...)
        4. Sort user      - move to the department OU and add groups
        5. Return to staging - if department is unknown or sort fails, leave
                               the account in 1NewUserStaging (ID cleared)

    The main loop runs this pipeline once per user, not in batch phases.
...
Write-Log "=== Provisioning run finished - $($PendingItems.Count) user(s) processed ==="
